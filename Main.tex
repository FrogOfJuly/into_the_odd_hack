...
\hrule \vspace{2mm}
\begin{center}
 Welcome another \emph{Into the Odd} hack that is tuned for the Realm of ???, a patch of land around a lake, that until recently had exactly zero mountains in the middle of it. With the addition of Magic, Injuries, and Quirks, it became its own system
...
\begin{itemize}
    ...
    \item[\ding{170}] Half, rounded up, of \texttt{d12} you assigned to \texttt{Vigor} is the amount of \texttt{Serious Injuries} the knight can survive before dying.
...
% can be found on Kerkenna and, though not frequently, knights known to employ it during their endeavours. Some knights are \textbf{Mages} and some are \textbf{Witches}.
...
\begin{itemize}[nosep]
    ...
    \item \textbf{Treated} \texttt{injury} occupies the slot, but does not apply \texttt{Affliction}
...
\begin{itemize}[nosep]
    ...
    \item \textbf{Dismount} a foe(+d6 to damage dice)
...
Dice that determine a knight's \texttt{Vigor} also define the amount of \texttt{Serious and Lethal Injuries} they can sustain.
...
\begin{itemize}[nosep]
    \item[\ding{170}] If a knight lost \texttt{6, 7, 8, or 9} \texttt{Vigor} through \texttt{Damage} -- take a \texttt{Serious Injury}.
    \item[\textbf{\textdagger}] If a knight lost \texttt{10 or more} \texttt{Vigor} through \texttt{Damage} -- take a \texttt{Lethal Injury}.
    \item[\ding{170}] If a knight lost half or more of their remaining \texttt{Vigor}, but not all of it -- take a \texttt{Serious Injury}.
    \item[\textbf{\textdagger}] If a knight lost all their \texttt{Vigor} or more through \texttt{Damage} -- take a \texttt{Lethal Injury}.
\end{itemize}
If more than one condition applies, take the worst one.

\subsection*{Consequences of an Injury} When knight suffers an \texttt{Injury} it fills corresponding \texttt{Injury slot}, recording a die that caused the injury.
If a knight is out of those slots, fill one with greater lethality. If they are out of those, the knight is \textbf{Slain}.

Each injury comes with \texttt{Afflictions}; after filling a slot, roll \texttt{d4} and consult the table below.
...
Each time a knight does an hour of \texttt{Short Rest}, they restore \texttt{Focus} up to their maximum minus the number of hours of \texttt{Short Rest} they already had this day.
...
\paragraph{Dealing with Trauma} removes it completely, but leaves a permanent mark: acquire a \texttt{Quirk}(p.\pageref*{sec:quirks}) as if a knight healed from an \texttt{Injury}.
...
The power of a mage is represented by a \texttt{Spellweaving Pool}: a set of d6 dice with size equal to the value of their \texttt{Clarity}.
...
If in doubt, know that this roll is affected by \texttt{Focus} and \texttt{Stress} mechanics.

There are many spells one can learn during their mage training, but regardless of syllabus, but all spellweavers know \textit{Detect Magic} by heart.
...
They persist until spent, and count as in \texttt{Spellweaving Pool}. The total amount of dice in the pool can never exceed the value of the mage's \texttt{Clarity}.
...
\paragraph{Overnight Rest} also restores up to \texttt{3} dice.

\paragraph{Consuming Spur} refills \texttt{Spellweaving Pool}.
...
You can learn how a spell unknown to you works. Upon discovery do a \texttt{Clarity} check, if successful, ask Referee if you can learn this spell and, if you can, how many times encountering this spell in the wild is enough.
...
It asked you to receive a gift. You accepted.

Fever was gone in a day. Swiftness of your recovery and strength you gathered amazed anybody who ever saw an infected wound like yours.
...
Flash of agony and its gone. Unnatural motions of muscles and skin are the only evidence of where it is.
...
You lose \texttt{1} \textit{maximum} \texttt{Clarity} and now have a \texttt{Spellweaving Pool} with a \texttt{d4} in it.
...
\begin{itemize}[nosep]
    ...
    For each \texttt{20} \texttt{Clarity} lost -- lose \texttt{1} \textit{maximum} \texttt{Clarity} and add an additional \texttt{d4} to the pool.
...
Once the splitting event is occurred, you switch to the new personality immediately. 
    ...
    Add this die to you \texttt{Spellweaving pool}.
...
There is a problem there that it's not clear what this pool represents outside of combat. 
