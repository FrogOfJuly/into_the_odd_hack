...
Find a Referee, create a character, decide if they are a mage, a witch, or neither, and go for it.
...
\newpage
\section*{\centering \fontsize{35}{45}\selectfont Character Creation}
\vspace*{0.4 cm}
...
Each character has three attributes that describe his body, mind, and soul.
...
Each of those rolls determines one of the character's \texttt{Virtues}; you choose which is which and discard the remaining one. You can use the character sheet on page \pageref*{sec:ch_sheet}.

Then, determine the character's capacity for \texttt{Injury}(\texttt{Injury Slots}), \texttt{Focus} and \texttt{Stress} according to the steps below.

\colbox{
\begin{itemize}[leftmargin=*]
    \item[\ding{170}] Half, rounded up, of \texttt{d12} you assigned to \texttt{Vigor} is the amount of \texttt{Severe Injuries} the character can survive before dying.
    \item[\textdagger] Half, rounded up, of \texttt{d6} you assigned to \texttt{Vigor} is the amount of \texttt{Lethal Injuries} the character can survive before dying.
    \item[\ding{84}] The value of \texttt{d6} you assigned to \texttt{Clarity} is maximum \texttt{Focus} the character has.
    \item[\ding{118}] Half, rounded up, of \texttt{d12} you assigned to \texttt{Spirit} is maximum \texttt{Stress} the character can endure before suffering a \texttt{Trauma}, minimum is \texttt{2}.
\end{itemize}
}
\paragraph{Guard.} Roll \texttt{d6} to determine the maximum \texttt{Guard(GD)} the character has.

\paragraph{Choose a Passion. } Something dear to the character's heart, or something weird that allows them to shrug off a failure.
...
When a character suffers an \texttt{Injury}, fill an \texttt{Injury slot} following the steps described below.
...
\end{itemize}
If a character suffers an injury and all corresponding slots are full, fill one with greater lethality. If they are out of those, the character is \textbf{Slain}.

\subsection*{Weapons \& Gear}

When picking gear for a character, consult with the Referee, but the rule of thumb is as follows: you have a budget of 12 points; these points can be spent on weapons and armour. 
...
Usually it makes sense to limit starting equipment(p.\pageref*{sec:gear}) to be common rarity with a single uncommon item as a prized possession of your character.

\subsection*{Laying the path}

Character is more than just stats, they lived their life in certain way and it should be somehow reflected.
To complete the character do the following steps: create or choose a background; choose a path; choose a trait(p.\pageref*{sec:martial_traits}) or an incomplete path.
...
\newpage
% \section*{\centering \fontsize{35}{45}\selectfont Paths Walked}
% \vspace*{0.4 cm}
...
These are inspirations of how your life was long before current events. 

\begin{center}
  \rowcolors{2}{gray!25}{white}
    \begin{tabular}{|c|p{6.4cm}|}
        \hline
        \textbf{Roll} & \textbf{Life long before} \\
        \hline
        \texttt{1} & Child of streets\\
        \texttt{2} & Runaway prisoner \\
        \texttt{3} & Acquired freedom through bravery\\
        \texttt{4} & Escaping ruined reputation\\
        \texttt{5} & Victim of political conflict\\
        \texttt{6} & Traded to a wizard in childhood\\
        \texttt{7} & Survivor of failed overseas campaign\\
        \texttt{8} & Raised on a smuggler vessel\\
        \texttt{9} & Child of exiles\\
        \texttt{10} & Cult's object of worship \\
        \texttt{11} & Survivor of disastrous ritual \\
        \texttt{12} & Lost heir of a fallen kingdom\\
        \hline
    \end{tabular}
\end{center}

You might have something left from those times, an item, a person you know or maybe somebody searches for you with unclear intent.

Also, throughout early years its possible to pick up spellweaving or be aligned with a patron. 
It is reasonable to have a pool with single \texttt{d4} and know \emph{Detect Magic} or recognise the patrons's presence and being able to use their cantrips.

Consult Referee when in doubt.

\subsection*{Path of Knight}

If you choose to be a knight -- you know all three \texttt{Feats}: \texttt{Smite}, \texttt{Focus} and \texttt{Deny}. 
They can perform them during combat, details of which are on page \pageref*{sec:combat}.

\subsection*{Path of Spellweaver}

One who has been taught proper spellweaving gets a \texttt{Spellweaving Pool} at character creation. 
Rules for spellweaving pool, spellcasting and list of available spells are on p.\pageref*{sec:spellweaving}.  

\subsection*{Path of Witch}

The witch get their patron at character creation. Rules for witchcraft are on p. \pageref*{sec:witchcraft}.

\newcolumn

\subsection*{Incomplete paths}

\paragraph{Ending squireship} was not an easy decision, but not every squire destined to become a knight. 
However, that does not mean all this time is wasted: you leaned a lot during those years. 

You know one \texttt{Feat} of your choice.

\paragraph{Lurking spellweaving} followed you around all your life, but either mages you encountered did not recognise the potential or you avoided training following your own reasoning.
You have a spellweaving pool with \texttt{3} \texttt{d6}'s in it. You restore \texttt{3} of your missing dice by indulging your passion or by having an overnight sleep. You know one cantrip and one spell in addition to \emph{Detect Magic}. 

If the total amount of dice in this spellweaving pool is less than half of your \texttt{Clarity} rounded down you can expand it.
Each time you learn a new spell, you add an additional \texttt{d6} to the pool.
...
There are two important resources each character has:
...
Sabotaging a roll is also possible.
...
Usually, a character has no restriction on how much \texttt{Focus} they can spend to enhance an action, but
determining how much focus is possible to spend to oppose a given action should be discussed with the Referee. 

\paragraph{Stress.} After a roll is made, a character can try to improve it by acquiring \texttt{Stress}.
...
perform an action: attack or gambit, always in that order.
...
\textbf{If} a character is injured and has no required injury slot left, they are \textbf{Slain}.
...
These special actions known by knights can be performed as described below. 
Using Feats risks the knight becoming \texttt{Fatigued}, which prevents them from using further \texttt{Feats} until they rest.
\colbox{
...
\paragraph{Makeshift treatment} \texttt{Gambit} allows one character to tend to another one, or one character to tend to themselves.
The one tending rolls a \texttt{Clarity Save}, and the one untended \texttt{Injury} becomes tended.
If that save fails, the targeted character must survive the pain of misjudged treatment by passing a \texttt{Vigor Save}.
If this one also fails, the injury is still treated, but increases in lethality, potentially killing the character.
...
Once a character sustains \texttt{Damage} to \texttt{Vigor} that does not reduce their \texttt{GD}, they gain a \texttt{Scar}.
...
Dice that determine a character's \texttt{Vigor} also control the amount of \texttt{Severe and Lethal Injuries} they can sustain.
...
\colbox{
\begin{itemize}[nosep, leftmargin=*]
    \item[\ding{170}] If a character lost from \texttt{4} to half of their remaining \texttt{Vigor} through \texttt{Damage} -- take a \texttt{Severe Injury}.
    \item[\textbf{\textdagger}] If a character lost half or more of their remaining \texttt{Vigor} -- take a \texttt{Lethal Injury}.
...
\end{itemize}
If injury is \textbf{Lethal} -- character falls unconsciousness until treated.
}
...
\subsection*{Trauma} 
When \texttt{Stress} reaches the limit a character can sustain, it causes \texttt{Trauma}: a footprint of a great effort in the form of a mental wound.
\texttt{Trauma} fills an \texttt{Injury slot} and kills the character by a heart attack if they are out of them.
...
\begin{multicols*}{2}
    ...
    Usually, non-player characters(NPCs), are tracked with less granularity than those controlled by players. 
    ...
    Each time a character or a friendly npc rolls \texttt{1} on their weapon die or \texttt{20} on a Virtue Save the Company accumulates one point of \texttt{Fear}. 
...
A character can restore their \texttt{Focus} by sheer willpower. They roll \texttt{d4} and lose that much \texttt{Clarity}. 
For each point of \texttt{Clarity} lost this way, a character restores one point of \texttt{Focus}.

A character can also reduce their \texttt{Stress} this way by rolling \texttt{d4} and losing that much \texttt{Spirit}.
...
Each character can restore their Focus through \texttt{Short Rest}, but the amount restored this way in total per day cannot exceed their maximum \texttt{Focus}.
...
\paragraph{Virtue restoration} during a \texttt{Short Rest} is possible, provided a character has required \textbf{Remedy}(p. \pageref*{sec:gear}).
...
\paragraph{Stress} is reduced by \texttt{1} each time a character sleeps full night, \textbf{Focus} is restored fully.

\paragraph{Virtues} can be restored by 3 points distributed however a character sees fit.
...
Still, in dire circumstances, a character can attempt to heal themselves or another character:

\colbox{
\begin{enumerate}[nosep, leftmargin=*]
    \item The one \textbf{healing} rolls a \texttt{Clarity} save to determine how to treat this type of \texttt{Injury} most effectively.
    \item The one \textbf{healed} rolls a \texttt{Spirit} save to see if the are able to withstand the pain.
\end{enumerate}
}
Only if both of the above saves are successful the \texttt{Injury} is healed.
Any one character can endure only \texttt{1} healing attempt per day.

\paragraph{Getting tougher and acquiring quirks.} After suffering through the agony of medicine without painkillers, a character is relieved from an injury completely.

If the maximum \texttt{Guard} of a character is less than the maximum value on a die that is marked on the \texttt{Injury Slot}, increase \texttt{Guard} by \textbf{1}.

Free the \texttt{Injury Slot} that was occupied by the healed \texttt{Injury}.

Choose or make the Referee choose a \texttt{Quirk} (p.\pageref*{sec:quirks}) for you that is described below and permanently add it to your character.
...
Trauma leaves a permanent mark: acquire a \texttt{Quirk}(p.\pageref*{sec:quirks}) as if a character healed from an \texttt{Injury}.
...
\begin{itemize}[nosep, leftmargin=*]
    ...
    \item[\ding{60}] \textbf{Hallucinogen} renders the character unable to perform basic tasks for six hours and allows them to fight all their inner demons at once.
    For each \texttt{Trauma}, they may roll a \texttt{Spirit} save. 
    After each failed save, the corresponding \texttt{Trauma} worsens and changes the injury slot to lethal; otherwise, it's removed, and the character receives a Quirk(p.\pageref*{sec:quirks}).
...
To weave a spell, weaver rolls any number of their dice from any available pools as an Action and then decides what to cast. This is a \texttt{Spellweaving roll}.
Only one spell can be cast per roll. If the spell is performed during combat -- pass a \texttt{Clarity} save or become \texttt{Fatigued}.
...
A character who has been taught spellweaving gets their \texttt{Spellweaving Pool} at character creation. 
They know any three spells from the list below in addition to \emph{Detect Magic} and one cantrip.
\begin{itemize}[nosep, leftmargin=*]
    \item \textbf{Dice} are all \texttt{d6}'s, and their amount is equal to the value of characters' \texttt{Clarity}.
...
\paragraph{Wisper.} You whisper a message to the target you can see. They hear you as if inside their ear. 
...
\paragraph{Mind Sliver.} Pick any number of dice, perform a ranged attack as if they are your weapon dice. 
...
Witches cast spells as manifestations of favors granted to them by their patrons.
Favour is spent after a spell is complete and is regained when a witch indulges the passions of their patrons.
If the spell is performed during combat -- pass a \texttt{Spirit} save or become \texttt{Fatigued}.
...
\begin{itemize}[nosep, leftmargin=*]
 ...
 In combat, it means that for the remainder of it, when you can use \texttt{Deny}. If you already could -- an additional die of your choice is discarded when you perform it.
    \item[\ding{108}] \textbf{Cursed Joke.} You tell your vilest joke and sprinkle enough magic to take it inside the target's head. Upon hearing it, target retracts themself from any activity they were doing.
 They seeks solitude and sit there in an increasingly catatonic state, after several minutes the recover.
 Their memory preserves a feeling of unpleasantness, but the content of the joke vanishes.
 If the target is in combat, their \texttt{Attacks} are \texttt{Impaired}.
\end{itemize}
...
And there was a great ramble and great fire. The iron earth collided with the iron sky. Aestur, the ravaging spirit, was born. The havoc ensorcelled it, and Aestus became determined to destroy. With great power, it broke off a mace out of the iron plane and went on a rampage through the universe.
...
\begin{itemize}[nosep, leftmargin=*]
    ...
    You can spend any amount of favor, upface the Mace's weapon die for each spent.
\end{itemize}
...
\begin{itemize}
    \item [\ding{108}] \textbf{Let iron eat your flesh.} From a piece of iron you hold splits a rough, razor-sharp splinter with jagged edges. You fling it for \texttt{d6 damage}. 
    If you rolled \textbf{6} on this damage die, you either: mortally wound(p.\pageref*{sec:hopefear}) your victim or deliver one \texttt{Severe Injury} or execute a Strong Gambit. 
...
\begin{itemize}
    \item [\ding{108}] \textbf{Hear my voice.} You clap your hands with deafening clang of metal. This is a melee attack with \texttt{d4 Blast} damage dice.
    Those in immediate vicinity of you who did not expect that do \texttt{Spirit} save and have their attacks impaired until your next turn if failed. 
    The sound can be heard from \textbf{very} far. After that is silence for a brief moment, if you speak, you can have focus-advantage to related \texttt{Clarity} or \texttt{Spirit} save.
...
You can choose the appearance of your soul and keep two pieces of your gear and one weapon.
    ...
    You create another character that from now on inhabits your body. 
...
\newpage
\section*{\centering \fontsize{35}{45}\selectfont Forgotten Spells}
\vspace*{0.4 cm}
...
\paragraph{The one gnarled to the bone.} You poor your vengeful power into your body. 
Once blade punctures your skin that power finds the living being that caused that harm to you and returns the favour.
...
Each time that damage courses an \texttt{Injury} that creature receives one also.
...
The can sense and hear things happening outside of the jar, but not see. When the jar is broken -- the escape and restore themselves to original state. 
...
\paragraph{Weapon mastery} manifests when a character develops an uncanny fascination with their weapons. 
...
\colbox{
\begin{enumerate}[nosep, leftmargin=*]
    \item Identify dice to combine among chosen dice
    \item Mark one of them as an accumulation die
    \item Add all values among identified dice and set accumulation die to that value
\end{enumerate}
If the accumulating die can't hold that value -- \emph{rethreading of the arcane} fails.
}

For example, if you chose to store two \texttt{d6}'s with values \texttt{2} and \texttt{3} you can instead store the as a single \texttt{d6} with a value \texttt{5} on it. 
...
Discount on learning new spells?
