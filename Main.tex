...
The tending knight rolls a \texttt{Clarity Save}, and the one untended \texttt{Injury} becomes tended.
If that save fails, the targeted knight must survive the pain of misjudged treatment by passing a \texttt{Vigor Save}.
...
\end{itemize}
Take the worse one if two or more apply. If injury is \textbf{Lethal} -- knight falls unconsciousness until treated.
}
...
\newpage
\section*{\centering \fontsize{35}{45}\selectfont Battle Flow \& Martial Prowess}
\vspace*{0.4 cm}
\label{sec:hopefear}
\begin{multicols*}{2}
    ...
    Also, it's rare that knights fight alone. 
    ...
    Additionally, if a knight attempts something risky -- it might make sense narratively to manifest their \texttt{Fear} as if they were opposed by some external forces. 
...
A knight who has been taught spellweaving gets their \texttt{Spellweaving Pool} at character creation. They know any three spells from the list below in addition to \emph{Detect Magic}.
\begin{itemize}[nosep, leftmargin=*]
...
Each time you roll maximum on your weapon die gain one favour for each point of damage that reduced targets \texttt{Vigor}.

\paragraph{Cantrips}
\begin{itemize}[nosep, leftmargin=*]
    \item[\ding{108}] \textbf{No, you can't have that.} You crush a small non-living object just outside of your reach. The force you impose is enough to snap a anything you can snap with your one hand.
    \item[\ding{108}] Any time you would attack while having one hand free of the weapon. You can summon a Mace of Aestur for the attack. It's a \textbf{Hefty} d6 weapon. 
...
\begin{itemize}
    ...
    \item [\ding{108}] \textbf{Remember your fears.} Let the presence of Aestur posses you for a split second to project power and rage around. 
    That intimidates the everybody who is near you. Treat the \texttt{Spirit} roll target makes to resist you as if it is opposed with \texttt{1} additional \texttt{Stress}. 
...
\begin{itemize}
    ...
    Those in immediate vicinity of you who did not expect that do \texttt{Spirit} save and have their attacks impaired if failed. The sound can be heard from \textbf{very} far.
...
These are spells that once were written in the Book of Gaub$^1$\footnotetext[1]{\href{https://lostpages.co.uk/presskit/sheet.php?p=book_of_gaub}{The Book of Guab by Lost Pages}}, but as hand's power dwindled -- they were lost to time.
...
\paragraph{The one trailing letters.} Perform a ritual, it kills you, with your last dying breath you utter a word that bind itself to an object.
...
Pick any amount of dice. For each die which rolled its maximum value -- ask a question, the answer has as many words as the value rolled.
...
Pick any amount of dice and calculate the sum of their values: thats how long spell lasts in hours. 
You can support the spell at twilight by picking more dice.

\paragraph{The one gnarled to the bone.} Some sort of returning damage spell. You wound me, you get the wound as well.

\paragraph{The one that is not there.} Antimemetic spell. You are unrememberable. 

\paragraph{The one bottling tears.} Put somebody in a jar? As a treat?
...
Roll \texttt{d8} as if you attacked yourself with a knife.
...
\paragraph{Your body is wasted on you. } Choose a target, roll \texttt{d20} and add their \texttt{Vigor}, this value represents how hard they resist you consuming their strength.
 Pick any amount of dice, add them together, add your \texttt{Vigor} that's your drain on them. 
 ...
 If they withstand your drain -- you lose \texttt{Vigor} equal to the difference. You can't influence the \texttt{d20} roll with \texttt{Focus} and \texttt{Stress}.
