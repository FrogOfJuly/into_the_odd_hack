...
When knight suffers an \texttt{Injury} -- fill corresponding \texttt{Injury slot} as described below.
\colbox{
\begin{enumerate}[nosep, leftmargin=*]
    \item Briefly describe an injury 
    \item Determine if injury is \texttt{Severe} or \texttt{Lethal}
    \item Record a damage die of the attack which caused the injury
...
TODO: describe equipment of the mage, witch and a knight without spellcasting.
...
For split second reaction, like avoiding sudden fall or dodging an arrow -- up to \texttt{2} \texttt{Stress} can be spent.
...
\paragraph{Stress \& Focus} can also be dealt with quickly, but require consuming certain items.
...
\paragraph{Focus} is restored, but with decreasing efficiency.
...
\paragraph{Virtue restoration.} During a \texttt{Short Rest} it is possible to restore Virtue, provided a knight has required supplies.
\begin{itemize}[nosep]
...
\begin{enumerate}[nosep, leftmargin=*]
    \item The \textbf{healing} knight rolls a \texttt{Clarity} check it determine how to treat this type of \texttt{Injury} most effectively.
    ...
    \item The \textbf{healed} knight who is treated rolls a \texttt{Spirit} check to withstand the pain.
...
\subsection*{Becoming a mage}

A knight who has been taught spellweaving since childhood gets their \texttt{Spellweaving Pool} at character creation. They know any three spells from the list below apart from \emph{Detect Magic}.
\begin{itemize}[nosep]
...
\paragraph{Soothing Visions. } Pick two dice both with the same value rolled as the target's \texttt{Stress}. Reduce their \texttt{Stress} by \texttt{1}.
