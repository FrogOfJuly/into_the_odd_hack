...
Some knights are \textbf{Spellweavers}(see p.\pageref*{sec:spellweaving}) and some are \textbf{Witches}(see p.\pageref*{sec:witchcraft}). If you choose neither consider \textbf{Martial traits}(p. \pageref*{sec:hopefear})
...
Weapon costs as much as its damage dice. For example, shortsword(2d6 damage) costs 12 points, knife(d6 damage) - 6 points. Each gear providing armour costs 2 per armour point, e.g. shield costs 6 points. 
...
% A knight gets an \texttt{Injury} according to conditions below. 
Damage from the \texttt{Attack} causes at most one \texttt{Injury}, see below, and one more can come from gaining a \texttt{Scar}.

\colbox{
\begin{itemize}[nosep, leftmargin=*]
    \item[\ding{170}] If a knight lost \texttt{5} or more \texttt{Vigor} through \texttt{Damage} -- take a \texttt{Severe Injury}.
...
Each untreated injury comes with \texttt{Affliction} according to the table below.
\begin{center}
  \rowcolors{2}{gray!25}{white}
...
\subsection*{Trauma} 
When \texttt{Stress} reaches the limit a knight can sustain -- it resets and leaves a \texttt{Trauma} behind: a footprint of a great effort in a form a mental wound.
\texttt{Trauma} fills an \texttt{Injury slot} and kills a knight by heart attack if they are out of those.
It has neither damage die nor \texttt{Afflication} and is dealt with differently(p.\pageref*{sec:healing}).
...
\newpage
\section*{\centering \fontsize{35}{45}\selectfont Fear \& Hope}
\vspace*{0.4 cm}
\label{sec:hopefear}
\begin{multicols*}{2}
    The battle rages as much on the field as in the warriors head. 
    Extinguishing fear and tending to dwindling hope is as important as finding weaknesses in enemy's armour. 

    Also its rare that knights fight alone. 

    \subsection*{Tracking NPC state}

    Usually, non-player characters(NPCs) and especially those who are not knights, are tracked with less granularity than those controlled by players. 
    And the main stats that is not tracked are \texttt{Injuries}, because its too much bookkeeping and \texttt{Stress} because of its get-out-of-jail nature. 
    ...
    Instead of tracking \texttt{Stress} of hostile NPCs Referee uses \texttt{Fear}, and \texttt{Hope} can be used to do the same for knights' allies.

    \subsection*{Fear}

    Each time a knight or a friendly npc rolls \texttt{1} on their weapon die or \texttt{20} on a Virtue Check the Company accumulates one point of \texttt{Fear}. 
    This is a resource, but it is spent not by knights, but by the Referee. 

    % \newcolumn{}

    Any time a hostile NPCs might want to use \texttt{Stress} -- Referee can decide that they spent \texttt{Fear} of the Company as if they were gaining \texttt{Stress}. 
    NPCs don't acquire \texttt{Trauma}s so its for the Referee to decide how much it is suitable to spend.

    Additionally, if knight attempts something risky -- it might make sense narratively to manifest their \texttt{Fear} as if they were opposed by some external forces. 

    Fear is the property of the Company, so it is persistent across passage of time and scenes.

    \subsection*{Hope}

    Each time an enemy of the Company rolls \texttt{1} on their weapon die or \texttt{20} on a Virtue Check the Company accumulates one point of \texttt{Hope}.

    This is a resource, but this time -- for the Company. Any player can use \texttt{Hope} points as \texttt{Focus} for themselves or \texttt{Stress} for friendly NPCs. 
    Spending \texttt{Hope} does not increase \texttt{Stress} and thus can't cause \texttt{Trauma}.

    Hope does not persist across fights and scenes, knights should convert it to \texttt{Focus} for themselves or their \texttt{NPCs}.
    ...
    During fights you can spend your \texttt{Guard} to move in strange dance of blade and fury. 
    When deciding what dice to spend on Gambits, you can use dice with value below \texttt{4} as long as you subtract the difference from your \texttt{Guard}.

    \paragraph{Weapon mastery} manifests when a knight develops an uncanny fascination with their weapons. 
    They treat them not as mere tools of the craft, but as partners or the most trustworthy friends.
...
\colbox{
Spealwaeving pool is described by
...
A knight who has been taught spellweaving gets their \texttt{Spellweaving Pool} at character creation. They know any three spells from the list below additionally to \emph{Detect Magic}.
\begin{itemize}[nosep, leftmargin=*]
