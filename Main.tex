...
A character can't spend \texttt{Stress} if they are at maximum \texttt{Stress}. 
After resolving an action, if their \texttt{Stress} is at maximum: 
    reduce it by \textbf{1}, 
    they lose \texttt{d6} \texttt{Spirit} and 
    suffer a \textbf{Trauma}(p.\pageref*{sec:injuries}).
% TODO: 
...
\subsection*{Trauma} 
When \texttt{Stress} reaches the limit a knight can sustain -- it causes \texttt{Trauma}: a footprint of a great effort in a form a mental wound.
\texttt{Trauma} fills any \texttt{Injury slot} and kills a knight by heart attack if they are out of them.
It has neither damage die nor \texttt{Afflication} and is dealt with differently (p.\pageref*{sec:healing}) 
% leaves behind a Quirk(p.\pageref*{sec:quirks}).
...
A knight can restore their \texttt{Focus} by shere willpower. They roll \texttt{d6} and lose that much \texttt{Clarity}. 
For each point of \texttt{Clarity} lost this way, a knight restores one point of \texttt{Focus}.

A knight can also reduce their \texttt{Stress} this way by rolling \texttt{d6} and lost that much \texttt{Spirit}.
If at least one point of \texttt{Clarity} was lost this way they reduce their \texttt{Stress} by \texttt{1}.

% and \texttt{1} during a \texttt{breather} by acquiring \texttt{1} \texttt{Stress}.
...
% During the \texttt{Short rest}, each knight can restore \texttt{Focus} that amounts to no more than their maximum \texttt{Focus} in total.
...
Each time you roll with \texttt{Focus} you can \texttt{Upface} on die before choosing what die to copy. 
