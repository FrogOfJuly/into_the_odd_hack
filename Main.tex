...
\begin{enumerate}[nosep, leftmargin=*]
    ...
    \item Combatants attacking the same target roll at the same time. Don't mix the dice: \texttt{Focus} and \texttt{Stress} applies individually.
...
For gambits other than \texttt{Bolster}, \texttt{Move} and \texttt{Patch a Wound}, the target receives a \texttt{Vigor Save} to ignore the effect.
...
\begin{itemize}[nosep]
    ...
    \item \textbf{Patch a Wound} on yourself or on ally nearby
...
\paragraph{Patching a Wound} \texttt{Gambit} allows one knight to tend to another one, or one knight to tend to themselves.
...
\begin{enumerate}[nosep, leftmargin=*]
    \item Roll a single \texttt{d20} against healing knight's \texttt{Clarity}, below or equal is a success. No \texttt{Focus} or \texttt{Stress} can be used, as the knight has no mastery over the craft of healing.
...
For each point of \texttt{Stress} you have -- treat those \texttt{Virtue} checks as if they were opposed with an additional \texttt{Stress}.
...
To weave a spell, a weaver rolls any number of dice as an Action from that set and then decides what to cast. This is a \texttt{Spellweaving roll}.
...
If in doubt, know that this roll is affected by \texttt{Focus} and \texttt{Stress}.
...
TODO: describe spellweaving pool more formally. 
...
They persist until spent, and count as in \texttt{Spellweaving Pool}. The total amount of dice in the pool can never exceed the value of the mage's \texttt{Spellweaving Pool} size.

\subsection*{Replenishing strength}
...
\begin{itemize}[nosep]
    ...
    For each \texttt{20} \texttt{Clarity} lost -- lose \texttt{1} \textit{maximum} \texttt{Clarity}, increase pool size by \texttt{1} and add an additional \texttt{d4} to the pool.
...
Once the splitting event is occurred, you switch to the new personality immediately. 
    ...
    You can't refill \texttt{Spellweaving pool} by resting or indulging a passion. If you didn't have a \texttt{Spellweaving pool} before you get one. 
    ...
    Add this die to you \texttt{Spellweaving pool}. You add this dice even if your pool is full. 
...
That complicates combat a bit, maybe its too much. 
...
\subsection*{Crits on Virtue checks}
...
\paragraph{You never learned.} Record each time you are \texttt{Fatigued}. Each fourth time reset the count, resist the Fatigue and gain \texttt{1} \texttt{Stress}.
...
\end{itemize}
Melee hefty weapons can be two-handed: add \texttt{d4} to their damage dice.
...
\paragraph{IDEA} It would be cool to restore or spent guard as a gambit. 
...
\paragraph{IDEA} Everybody gets five injury slots. Those are guarded by \texttt{d4, d6, d8, d10, d12} respectively. 
