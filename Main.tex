...
\paragraph{Guard.} Roll \texttt{d6} to determine the maximum \texttt{Guard} the knight has.
...
\begin{enumerate}[nosep, leftmargin=*]
    ...
    \item Combatants attacking the same target roll at the same time. Don't mix the dice yet: \texttt{Focus} and \texttt{Stress} applies individually.
...
\begin{itemize}[nosep]
    ...
    \item \textbf{Patch a Wound} of yourself or an ally at arms reach
...
\begin{itemize}[nosep]
    ...
    \item \textbf{Greater effect} such as disarming an item, breaking a wooden shield, or removing a helm. Bolster can't add more than \texttt{1} \texttt{Damage}.
\end{itemize}
...
Each time a knight sustains \texttt{Damage} to \texttt{Vigor} which does not reduce their \texttt{Guard} they gain a \texttt{Scar}.
Roll the damage die again, add amount of your \texttt{injuries} and consult the table below. 
...
\begin{itemize}[nosep]
    \item[\ding{60}] \textbf{Spur} restores \texttt{Focus} and \texttt{Spellweaving} dice
    \item[\ding{60}] \textbf{Sedative} reduces \texttt{Stress} by \texttt{2}
\end{itemize}
...
\begin{itemize}[nosep]
 ...
 In combat, it means that for the remainder of the combat, when you \texttt{Deny} -- an additional die of your choice is discarded.
...
\begin{itemize}[nosep]
    ...
    \item[\ding{108}] \textbf{Grow my beloved, grow.} You put a new life into dead vegetation with your touch.
...
\begin{itemize}[nosep]
    \item Each time you do \texttt{Overnight Rest} if your \texttt{Guard} is greater than \texttt{1} reduce it by \texttt{2}.
...
I think making up the difference between the die and the gambit-value.
...
Like a Quirk, but purely positive. Can they be one time things? 
...
\paragraph{They never learned.} Record each time you are \texttt{Fatigued}. Each fourth time -- reset the count and gain \texttt{1} \texttt{Stress} instead.
