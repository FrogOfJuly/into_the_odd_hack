...
\begin{itemize}[nosep]
    ...
    \item \textbf{Healed} \texttt{injury} is removed entirely
\end{itemize}
...
If one character can affect the roll of another destructively, for each \texttt{Focus} spent, the roll accumulates \texttt{Disadvantage}.
...
Direct confrontation, like overpowering a melee attack -- up to \texttt{2} \texttt{Focus} can be spent.
Indirect opposition, like falling prone to avoid an arrow -- up to \texttt{1} \texttt{Focus} can be spent.
...
One can also acquire \texttt{Stress} to oppose an action after the roll is made the same way as it works with \texttt{Focus}.

Opposing characters decide how much \texttt{Stress} they spend simultaneously.

A character can't spend \texttt{Stress} if they are at maximum stress.
...
\colbox{
\begin{enumerate}[nosep, leftmargin=*]
    \item Declare and spend \texttt{Focus}, target declares opposition and matching \texttt{(Dis)Advantages} are removed.
    \item All combatants that attack the same target roll simultaneously, but do not mix the dice together as \texttt{Focus} and \texttt{Stress} are applied individually.
    \item Remove extra dice added through \texttt{Focus}.
    \item Declare \texttt{Stress} gain, target declares opposition, remove matching \texttt{(Dis)Advantages}, roll added dice.
...
Each time a knight sustains \texttt{Damage} to \texttt{Vigor} that does not reduce their \texttt{Guard} they gain a \texttt{Scar}.
Roll a damage die that did that \texttt{Damage} again and consult the table below.
...
\begin{center}
  ...
  \label{scars}
    \begin{tabular}{|c|p{5.7cm}|}
        ...
        \texttt{2} & Wood cracks as shield fails. If your max \texttt{GD} is \texttt{2} or less, increase it by \texttt{d6}. \\
        ...
        \texttt{11} & Death searches for you. Gain \texttt{4} \texttt{Stress}. \\
...
A knight gets an \texttt{Injury} according to conditions below. Each \texttt{Attack} causes one \texttt{Injury} at most.
