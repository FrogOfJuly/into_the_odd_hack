...
If the action is contested by \texttt{Focus} or \texttt{Stress}, opposing characters reveal how much they spend simultaneously each time. 

% After resolving an action, if their \texttt{Stress} is full: 
...
\end{itemize}
Take worse one if two or more apply. If injury is \textbf{Lethal} -- knight falls unconsciousness until treated.
}
...
\paragraph{Haste.} Pick any number of dice that amount to target's Vigor or greater.
...
\paragraph{Blade ward.} Pick any amount of dice, mark them as spent, but the are unrecoverable until discarded from the spell.
Each time an attack would hit you for damage that is exactly equal to value of any picked dice -- treat the attack as if it was evaded. 
Discard the matching die. The spell persists until dismissed or all dice are spent.

\paragraph{Mind sliver.} Pick any amount of dice, perform a ranged attack as if those dice are your weapon dice. 
However, the damage from this spell is dealt to \texttt{Spirit} or \texttt{Clarity} instead of \texttt{Vigor}. 
If half or more \texttt{Virtue} is lost as a result of this spell -- target's attacks are \texttt{Impaired} until your next turn.
...
\paragraph{Strength in suffering.} When you attack, upface a die if you have at lest one \texttt{Injury Slot} filled.
