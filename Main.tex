...
\begin{multicols*}{2}
    ...
    Instead of tracking \texttt{Stress} of hostile NPCs Referee uses \texttt{Fear}. The players can use \texttt{Hope} to do the same for knights' allies.

    \subsection*{Fear}

    Each time a knight or a friendly npc rolls \texttt{1} on their weapon die or \texttt{20} on a Virtue Save the Company accumulates one point of \texttt{Fear}. 
    ...
    Each time an enemy of the Company rolls \texttt{1} on their weapon die or \texttt{20} on a Virtue Save the Company accumulates one point of \texttt{Hope}.
...
A knight can restore their \texttt{Focus} by shere willpower. They roll \texttt{d4} and lose that much \texttt{Clarity}. 
For each point of \texttt{Clarity} lost this way, a knight restores one point of \texttt{Focus}.

A knight can also reduce their \texttt{Stress} this way by rolling \texttt{d4} and lose that much \texttt{Spirit}.
...
During your sleep pass a \texttt{Clarity} save and a \texttt{Spirit} saves to successfully deal with \texttt{Trauma}. 
...
\begin{itemize}[nosep, leftmargin=*]
    ...
    \item[\ding{60}] \textbf{Hallucinogen} renders knight unable to perform basic tasks for six hours and allows them to fight all their inner deamons at once.
    For each \texttt{Trauma} they may roll a \texttt{Spirit} save. 
    After each failed save - corresponding \texttt{Trauma} worsens and changes injury slot to lethal, otherwise its removed and knight receives a Quirk(p.\pageref*{sec:quirks}).
...
\paragraph{Helping hands.} You summon a pair of hands that try to help you. 
...
In combat they can wield either \textbf{two non-Hefty} weapons or \textbf{one Hefty}. 
They are not very strong, so nothing heavier is feasible. You can still benefit from only one shield. 
They last for a task you summoned them to fullfil or for one combat. Slowly dissipate when there is nothing for them to do.
